...
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
  ...
  xml.updated(blog(:works).articles.first.date.to_time.iso8601) unless blog(:works).articles.empty?
  xml.author { xml.name "株式会社フィルイン" }

  blog(:works).articles[0..4].each do |article|
    xml.entry do
      ...
      xml.image {
        xml.url URI.join(site_url, article.data.cover)
        xml.title article.title
